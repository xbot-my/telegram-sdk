...
// ResponseFormat removed; using formattable result methods instead
use XBot\Telegram\Http\Response\TelegramResponse;

class DummyHttpClient implements ClientContract
{
    ...
    {
        return '000:FAKE_TOKEN_DUMMY_______________________________';
    }
...
class ExposedGroup extends BaseEndpoint
{
    public function __construct()
    {
        parent::__construct(new DummyHttpClient(), 'bot');
    }
    
    public function __invoke(...$args): mixed { return null; }
    
    public function doPrepare( array $params ): array
    {
        return $this->prepareParameters($params);
    }
    
    public function doExtract( array $params ): array
    {
        $files = $params;
        $out   = $this->extractFiles($files);
        
        return [ $files, $out ];
    }
    
    public function formatNow( mixed $data ): mixed
    {
        return $this->formatResult($data);
    }
}

it('prepares parameters and formats via wrapper', function () {
    $g = new ExposedGroup();
    ...
    // format using wrapper methods
    $obj = $g->formatNow([ 'id' => 1 ])->toObject();
    expect($obj)->toBeObject()->and($obj->id)->toBe(1);

    $arr = $g->formatNow([ 'id' => 2 ])->toArray();
