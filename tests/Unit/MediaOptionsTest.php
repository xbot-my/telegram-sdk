...
it('passes options for sendPhoto and triggers upload for local file', function () {
    ...
    $msg = $bot->sendPhoto(1, $file, ['caption' => 'cap', 'parse_mode' => 'HTML', 'disable_notification' => true]);
...
it('passes options for other media endpoints', function () {
    ...
    $bot->sendVideo(1, $file, $opts);
    $bot->sendAudio(1, $file, $opts);
    $bot->sendDocument(1, $file, $opts);
    $bot->sendVoice(1, $file, $opts);
    $bot->sendAnimation(1, $file, $opts);
    ...
    foreach (['sendVideo','sendAudio','sendDocument','sendVoice','sendAnimation'] as $m) {
        ...
        expect($seen[$m]['params']['disable_notification'] ?? null)->toBeTrue();
...
it('sendLocation and sendContact pass options', function () {
    ...
    $bot = new TelegramBot('t', $client);
    $bot->sendLocation(1, 10.0, 20.0, ['disable_notification' => true]);
    $bot->sendContact(1, '123', 'Jane', ['last_name' => 'Doe']);
