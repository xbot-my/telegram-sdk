...
use XBot\Telegram\Http\Client\Config;
...
it('throws on invalid config', function () {
    // missing token
    expect(fn() => \XBot\Telegram\Http\Client\Config::fromArray([]))->toThrow(InvalidArgumentException::class);
    
    // bad base url
    $cfg = new \XBot\Telegram\Http\Client\Config('12345678:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdef0123456789', 'not-a-url');
