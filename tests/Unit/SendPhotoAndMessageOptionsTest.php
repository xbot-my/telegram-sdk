...
class FakeHttpClientMedia implements ClientContract
{
    ...
    {
        $this->lastCall = [ $method, $parameters ];
    ...
    {
        $this->lastCall = [ $method, $parameters, $files ];
    ...
    {
        return '000:FAKE_TOKEN_MEDIA___________________________';
    }
...
it('sendMessage supports options and returns array', function () {
    ...
    $res = $bot->sendMessage(9, '<b>HTML</b>', [ 'parse_mode' => 'HTML', 'disable_notification' => true ])->toArray();
...
it('sendPhoto switches to upload when given file path', function () {
    ...
    $res = $bot->sendPhoto(9, $tmp, [ 'caption' => 'pic' ])->toArray();
